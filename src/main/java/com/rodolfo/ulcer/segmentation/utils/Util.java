...
public class Util {
    ...
    public static File createDatasourceFile(String path, MethodEnum method, Configuration configuration) {
        ...
        if(method.name().equals("DISF")) {

            dFile = new File(dirPath.concat(path).concat("/").concat(configuration.getDatasourceDISFName()));
        }

        if(method.name().equals("ERS")) {

            dFile = new File(dirPath.concat(path).concat("/").concat(configuration.getDatasourceERSName()));
        }

        if(method.name().equals("TURBOPIXEL")) {

            dFile = new File(dirPath.concat(path).concat("/").concat(configuration.getDatasourceTURBOPIXELName()));
        }

        if(method.name().equals("SH")) {

            dFile = new File(dirPath.concat(path).concat("/").concat(configuration.getDatasourceSHName()));
        }

        if(method.name().equals("SNIC")) {

            dFile = new File(dirPath.concat(path).concat("/").concat(configuration.getDatasourceSNICName()));
        }
    ...
    public static File createMinMaxFile(MethodEnum method, Configuration configuration) {
        ...
        if(method.name().equals("DISF")) {
            ...
            dFile = new File(dirPath.concat(configuration.getMinMaxDISFName()));
        }

        if(method.name().equals("ERS")) {

            dFile = new File(dirPath.concat(configuration.getMinMaxERSName()));
        }

        if(method.name().equals("SNIC")) {

            dFile = new File(dirPath.concat(configuration.getMinMaxSNICName()));
        }
    ...
    public static File createMlModelFile(MethodEnum method, Configuration configuration) {
        ...
        if(method.name().equals("DISF")) {
            ...
            dFile = new File(dirPath.concat(configuration.getMlModelDISFName()));
        }

        if(method.name().equals("ERS")) {

            dFile = new File(dirPath.concat(configuration.getMlModelERSName()));
        }
        
        if(method.name().equals("SNIC")) {

            dFile = new File(dirPath.concat(configuration.getMlModelSNICName()));
        }
