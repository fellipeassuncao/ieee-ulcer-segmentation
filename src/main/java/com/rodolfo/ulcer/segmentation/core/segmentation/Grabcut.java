...
public class Grabcut implements Process {
    ...
    private Mat grabCutFinalBinarySegmentation;
    private Mat grabCutFinalUlcerSegmentation;
    ...
    public Grabcut(Image image, Configuration conf, Mat outlineFilled, Mat skeleton) {
        ...
        this.grabCutFinalUlcerSegmentation = new Mat(this.image.getSize(), this.image.getType(), Scalar.WHITE);
    }
    ...
    public void process() {
        ...
        this.image.getImageWithoutReflection().copyTo(this.grabCutFinalUlcerSegmentation, aux1);
        this.image.getImageWithoutReflection().copyTo(this.grabCutFinalUlcerSegmentation, aux2);
    }
    ...
    public void createGrabCutFinalBinarySegmentation() {

        log.info("Criando a imagem binária final a partir do método GrabCut");

        this.grabCutFinalBinarySegmentation = Mat.zeros(this.grabCutFinalUlcerSegmentation.size(), opencv_core.CV_8UC1).asMat();

        Mat segmentationAux = new Mat(this.grabCutFinalUlcerSegmentation.rows() + 2, this.grabCutFinalUlcerSegmentation.cols() + 2, opencv_core.CV_8UC1, new Scalar(255.0));
        Mat aux1 = Mat.zeros(this.grabCutFinalUlcerSegmentation.size(), opencv_core.CV_8U).asMat();
        Mat comparator1 = new Mat(1, 1, opencv_core.CV_8U, Scalar.WHITE);

        opencv_core.compare(this.grabCutFinalUlcerSegmentation, comparator1, aux1, opencv_core.CMP_EQ);
        ...
        opencv_core.bitwise_not(OpenCV.croppMat(largeContour, 1, 1, segmentationAux.rows() - 2, segmentationAux.cols() - 2), this.grabCutFinalBinarySegmentation);
    }
    ...
    public void createHumanMaskWithLabeledContour() {
        ...
        if(this.image.getDirectory().hasLabeledImagePath()) {
            ...
            opencv_core.add(this.grabCutHumanMask, this.grabCutHumanMask);
        }
    }
}
