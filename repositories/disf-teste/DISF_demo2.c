...
int main(int argc, char* argv[])
{
    ...
    n_0 = atoi(argv[2]);
    n_f = atoi(argv[3]);
    dirlabel =atoi(argv[4]);
    dirborder = atoi(argv[5]);
    ...
    writeImagePGM(label_img, dirlabel);
    writeImagePGM(border_img, dirborder);
    printf("Verificando se o caminho foi passado corretamente por parâmetro:", dirlabel);
    freeImage(&label_img);
...
void usage()
{
    printf("Usage: DISF_demo <1> <2> <3> <4> <5>\n");
    printf("----------------------------------\n");
    printf("INPUTS:\n");
    ...
    printf("<4> - Caminho para salvar labels (labels.pgm)");
    printf("<5> - Caminho para salvar borders (borders.pgm)");
